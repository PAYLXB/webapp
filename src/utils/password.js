...
let Hash_txt = function() {
  ...
  pv.$SHA1 = function(str) {
    //Encode sting to SHA-1
    if (str == "") return null;
    var tmstr = pv.txt2ary(str);
    return pv.SHA1(tmstr, tmstr.length);
  };
  pv.$SHA256 = function(str) {
    //Encode sting to SHA-1
    if (str == "") return null;
    var tmstr = pv.txt2ary(str);
  ...
  pv.MD5 = function(buff, len) {
    ...
    if (len > 268435456 || len < 0) {
      return 0;
    }
    var block64 = Array();
    lenmod = len % 64;
    if (lenmod >= 56) {
      //Extend data to multiple of 512 bits
      lenmod -= 64;
    }
    buff[len] = 0x80;
    lenmod++;
    ...
    for (i_ = 0; i_ < ibuff; i_ += 64) {
      ...
      for (j_ = 0; j_ < 64; j_++) {
        ...
        leftrotate = safeadd(
          safeadd(tem0, md5_f),
          safeadd(const64r[j_], block64[md5_g])
        );
  ...
  pv.SHA1 = function(buff, len) {
    ...
    for (i_ = 0; i_ < ibuff; i_ += 64) {
      ...
      for (j_ = 0; j_ < 80; j_++) {
        ...
        temp = safeadd(
          leftr(tem0, 5),
          safeadd(sha_f, safeadd(tem4, safeadd(sha_k, block64[j_])))
        ); //杂凑运算
        tem4 = tem3;
        tem3 = tem2;
        tem2 = leftr(tem1, 30);
  ...
  pv.SHA256 = function(buff, len) {
    ...
    for (i_ = 0; i_ < ibuff; i_ += 64) {
      ...
      for (j_ = 16; j_ < 64; j_++) {
        //Extend chunk to 64 intergers
        tem0 =
          rightr(block64[j_ - 15], 7) ^
          rightr(block64[j_ - 15], 18) ^
          (block64[j_ - 15] >>> 3);
        tem1 =
          rightr(block64[j_ - 2], 17) ^
          rightr(block64[j_ - 2], 19) ^
          (block64[j_ - 2] >>> 10);
        block64[j_] = safeadd(
          block64[j_ - 16],
          safeadd(tem0, safeadd(block64[j_ - 7], tem1))
        );
      }
      tem0 = h0;
      tem1 = h1;
      ...
      for (j_ = 0; j_ < 64; j_++) {
        ...
        sha_t1 = safeadd(
          tem7,
          safeadd(sha_s1, safeadd(sha_ch, safeadd(prem64[j_], block64[j_])))
        );
        tem7 = tem6;
        tem6 = tem5;
        tem5 = tem4;
  ...
  pv.txt2ary = function(txt) {
    //Text convert to byte stream encode with utf-8
    var codeary = Array();
    var cc = 0;
    var ucode = 0;
    var ufcode, i, j;
  ...
  pv.ucs2utf8 = function(uc) {
    //Unicode character encode to utf-8
    var carray = Array();
    if (uc >= 0 && uc <= 0x7f) {
      //ASCII
      carray[0] = 1;
  ...
  var le2lex = function(A) {
    //Little endian integer to hex
    var d2e = [
      "0",
      "1",
      "2",
  ...
  var bi2lex = function(A) {
    //Big endian integer to hex
    var d2e = [
      "0",
      "1",
      "2",
