<template>
  <div id="page">
    <div class="wrap">
      <div class="close" @click="closeplate"></div>

      <div class="radio-box">
        <label class="flex-items-center">
          <img v-if="formData.commonCard == 1" src="../../assets/images/icon_chose_s@2x.png" alt />
          <img v-else src="../../assets/images/icon_chose_n@2x.png" alt />
          <input type="radio" v-model="formData.commonCard" value="1" />普通车牌
        </label>
        <label class="flex-items-center">
          <img v-if="formData.commonCard == 2" src="../../assets/images/icon_chose_s@2x.png" alt />
          <img v-else src="../../assets/images/icon_chose_n@2x.png" alt />
...
export default {
  ...
  mounted() {
    this.clickFirstWrap()
    console.log(this.$route.params.plateCode)
       if(this.$route.params.plateCode){
           this.$route.params.plateCode.split('').map((item,idx)=>{
                return  this.formData['num'+idx]=item
            })
    }
  },
  methods: {
    closeplate() {
      this.$emit("getplatelicense");
    },
    clickFirstWrap() {
      // 点击第一个输入框
